...
class PiecewiseFree(nn.Module):
    """EML(x) for x<=0, free quadratic for x>0."""
    ...
    def forward(self, x):
        ...
        if x.dim() == 2:
            a, b, c, d, alpha, beta, gamma = [p.unsqueeze(0) for p in [a, b, c, d, alpha, beta, gamma]]
...
results={}
print('PINN + PiecewiseFree | Device: %s'%device)
...
out={'experiment':'PINN + PiecewiseFree','config':{'epochs':5000,'lr':0.001},'results':results,'date':'2026-04-30'}
with open(os.path.join(os.path.dirname(__file__),'pinn_pwfree_results.json'),'w') as f: json.dump(out,f,indent=2)
print('\nSaved pinn_pwfree_results.json')
